...
public class PSMAttack : StateMachineBehaviour
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if (animator.GetCurrentAnimatorStateInfo(0).IsName("Player Special Attack State"))
        {
            ...
            else if (animator.GetComponent<PSMController>().RB2D.velocity.y > 1 && (!Input.GetKey(KeyBinding.KeyBindSetController(KeyBinding.KeyBindInstance.ControllerStringKeyUp)) || !KeyBinding.KeyBindInstance.SetAxisSign(KeyBinding.KeyBindInstance.ControllerStringKeyUp)) && CheckInput.Controller == true)                                                                                                //Altrimenti se la velocità è maggiore di 1 e non sto più schiacciando il pulsante spazio - Da un minimo di velocà di salto minimo - Permette di fare salti graduali
            {
                animator.GetComponent<PSMController>().RB2D.velocity += Vector2.up * Physics2D.gravity.y * (animator.GetComponent<PSMController>().ValueJump.lowJumpMultiplier - 1) * Time.deltaTime;       //Calcola la velocità definendo una direzione iniziale (UP) e moltiplicando per una gravitò maggiorata minore è il tempo di pressione del tasto
            }
